...
def load_image(img_path, saturation, contrast, bbox, cropped=False, black=False):
    image = Image.open(img_path).convert("RGB")

    image_crop = image.crop((bbox["xmin"], bbox["ymin"], bbox["xmax"], bbox["ymax"]))

    image_black = Image.new("RGB", image.size, (0, 0, 0))
    image_black.paste(image_black, (bbox["xmin"], bbox["ymin"]))
...
def save_json_results(output_dir, masks, scores, class_names, input_boxes, image, img_path):
...
def main(args):
    ...
    for filename in sorted(os.listdir(input_dir)):
        if filename.endswith('.png'):
            ...
            # image, image_crop, image_black = load_image(img_path, saturation=1.0, contrast=1.0, bbox=None, cropped=False, black=False)
            ...
            if input_boxes.size == 0:
                print(f"No objects detected, skipping {filename} for SAM2.")

                img = cv2.imread(img_path)
                cv2.imwrite(os.path.join(output_dir, filename), img)

                save_json_results(output_dir, None, None, None, None, image, img_path)
                ...
                save_json_results(output_dir, masks, scores, class_names, input_boxes, image, img_path)
